...
class TwitchAPI
{
    ...
    {
        $requestData = $this->httpAPI->get($this->clientID, "https://api.twitch.tv/helix/users?login=" . $name,
            $authenticationToken);
        try {
        ...
        $user = new TwitchUser($data->id, $data->login, $data->display_name, $data->type, $data->broadcaster_type,
            $data->description, $data->profile_image_url, $data->offline_image_url, $data->view_count);
        if (isset($data->email)) {
    ...
    {
        $requestData = $this->httpAPI->get($this->clientID, "https://api.twitch.tv/helix/users?id=" . $userID,
            $authenticationToken);
        try {
            $this->verifyData($requestData);
        } catch (TwitchAPIException $exc) {
    ...
    public function getVideoByVideoID($videoID)
    {
        ...
        $data = $requestData->data[0];
        return new TwitchVideo($data->id, $data->user_id, $data->title, $data->description, $data->created_at,
            $data->published_at, $data->url, $data->thumbnail_url, $data->viewable, $data->view_count, $data->language,
    ...
    public function getVideosByVideoIDArray(array $videoIDS)
    {
        ...
        foreach ($data->data as $d) {
            $vids[$vidPointer] = new TwitchVideo($d->id, $d->user_id, $d->title, $d->description, $d->created_at,
                $d->published_at, $d->url, $d->thumbnail_url, $d->viewable, $d->view_count, $d->language, $d->type,
                $d->duration);
            $vidPointer ++;
        }
        return $vids;
    ...
    public function getVideosByUserID($userID, $type = "all", $sort = "time", $period = "all", $amount = 20, $paginationCursor = null,
        $language = null)
    {
        if ($amount > 100) {
        ...
        foreach ($requestData->data as $data) {
            $vids[$vidPointer] = new TwitchVideo($data->id, $data->user_id, $data->title, $data->description,
                $data->created_at, $data->published_at, $data->url, $data->thumbnail_url, $data->viewable,
                $data->view_count, $data->language, $data->type, $data->duration);
            $vidPointer ++;
        }
    ...
    public function getVideosByGameID($gameID, $type = "all", $sort = "time", $period = "all", $amount = 20, $paginationCursor = null,
        $language = null)
    {
        if ($amount > 100) {
            throw new TwitchAPIException("Maximum Amount of Videos that can be returned by one Request is 100");
    ...
    public function getGameAnalyticsBuilder($authenticationToken, $gameID = null, $amount = 20, $type = null, $startedAt = null,
        $endedAt = null)
    {}
